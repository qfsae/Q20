...
//Grupo de assets
char Asset[] = "nightw.gd2";  File GAsset;  String STRAsset(Asset);
char Asset1[] = "night1.gd2";  File GAsset1;  String STRAsset1(Asset1);
...
public:
  ...
  void load_level(byte n) {
    ...
    File levelRes;
    String str(filename);
    levelRes = SD.open(str);
    GD.loadSDIO(levelRes);
    //GD.load(filename);
    //End RndMnkIII
...
static void welcome()
{
  StreamerSDIO stream;
  File welcomeRes;
   String str("nightw.gd2");
  welcomeRes = SD.open(str);
  
  GD.safeloadSDIO(welcomeRes);
  byte fade = 0;
  int t;
